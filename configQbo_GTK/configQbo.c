...
{
  printf("READ: age: %d, lang:%s, startWith: %s, token: %s, op_question: %s, volume: %d\n, geminiAPI: %d\n", conf.age, conf.language, conf.startWith, conf.tokenAPIai, conf.op_question, conf.volume, conf.geminiAPI);
...
void on_butt_exit_clicked (GtkButton *button, gpointer   user_data)
{
  ...
  if (dlgresponse == 1) {
    ...
    printf_conf();
    //    printf("CONF: age: %d, lang:%s, start: %s, token: %s.\n", conf.age, conf.language, conf.startWith, conf.tokenAPIai, conf.geminiAPI);
    writeConfig(&conf);
    gtk_main_quit();
  }
...
void on_combo_startWith_changed(GtkComboBox *widget, gpointer toggleOpQuestion)
{
  ...
  case 0:
    ...
    break;
  case 1:
    sprintf(conf.startWith, "interactive-dialogflow"); 
    gtk_widget_show( GTK_WIDGET(toggleOpQuestion));
    break;
  case 2:
    sprintf(conf.startWith, "interactive-gassistant");
    gtk_widget_show( GTK_WIDGET(toggleOpQuestion));
    break;
  case 3: 
   sprintf(conf.startWith, "gemini");
   gtk_widget_show( GTK_WIDGET(toggleOpQuestion));
   break;
  default:
    break;
...
void onAssistantTokenChange(GtkEntry *entry, gpointer   user_data)
{
  sprintf(conf.gassistant_proyectid, "%s", gtk_entry_get_text (entry));
}

void onGeminiTokenChange(GtkEntry *entry, gpointer   user_data){
  sprintf(conf.geminiAPI, "%s", gtk_entry_get_text (entry));
}
...
int main(int argc, char *argv[])
{
  ...
  GtkEntry *entryToken;
  GtkEntry *entryTokenAssistant;
  GtkEntry *entryGeminiAPI;
  ...
  else if (strcmp(conf.startWith, "interactive-dialogflow") == 0) {
    gtk_combo_box_set_active (GTK_COMBO_BOX(comboStartWith), 1);
    gtk_widget_show( GTK_WIDGET(toggleOpQuestion));
  }
  else if(strcmp(conf.startWith, "gemini") == 0) {
    gtk_combo_box_set_active (GTK_COMBO_BOX(comboStartWith), 1);
    gtk_widget_show( GTK_WIDGET(toggleOpQuestion));
  }
  else {
    gtk_combo_box_set_active (GTK_COMBO_BOX(comboStartWith), 2);
    gtk_widget_show( GTK_WIDGET(toggleOpQuestion));
  }
  ...
  // update assistant entry
  entryTokenAssistant = GTK_ENTRY(gtk_builder_get_object(builder, "entryTokenAssistant"));
  gtk_entry_set_text (entryTokenAssistant, conf.gassistant_proyectid);
  
  // update gemini entry
  entryGeminiAPI = GTK_ENTRY(gtk_builder_get_object(builder,"entryGeminiAPI"));
  gtk_entry_set_text (entryGeminiAPI, conf.geminiAPI);
